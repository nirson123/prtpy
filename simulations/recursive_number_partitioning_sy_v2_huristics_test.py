...
"""
(1) :  sum(S1) >= best_sum          => prone
(2) :  sum(S1^c) >= best_sum*(k-1)  => prone
(3) :  sum(S1) + I_i <= min_sum     => prone
(4) :  max(S) <= min_sum            => return
"""

def rnp(binner: Binner, numbins: int, items: List[any], huristics: List[int] = [1,2,3,4]) -> BinsArray:
    """
    Based on "Search Strategies for Optimal Multi-Way Number Partitioning", Michael D. Moffitt, 2013
    https://www.ijcai.org/Proceedings/13/Papers/099.pdf
    ...
    _, _, best_partition = _rnp_recursive(binner.new_bins(numbins), binner,
                                          ...
                                          max(binner.sums(initial_partition)), initial_partition, huristics)
...
def _handel_group(current_group, rest_of_items, best_sum, best_partition, binner, bins, current_bin, min_sum, huristics):
    # pruning
    current_group_sum = sum(map(binner.valueof, current_group))
    rest_of_items_sum = sum(map(binner.valueof, rest_of_items))
    if (current_group_sum >= best_sum and 1 in huristics) or \
            (rest_of_items_sum >= best_sum * (binner.numbins(bins) - current_bin - 1) and 2 in huristics) or \
            (any([current_group_sum + binner.valueof(item) <= min_sum for item in rest_of_items]) and 3 in huristics):
        return None
    ...
    if current_sum <= min_sum and 4 in huristics:
...
def _rnp_recursive(bins: BinsArray, binner: Binner, items: List[any], current_bin: int, min_sum: Number,
                   best_sum: Number, best_partition: BinsArray, huristics) -> (BinsArray, Number, BinsArray):
    """
    ...
    for current_group, rest_of_items in _all_sub_groups(items):
        group_result = _handel_group(current_group, rest_of_items, best_sum, best_partition, binner, bins, current_bin, min_sum, huristics)
